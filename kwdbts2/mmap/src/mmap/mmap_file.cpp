...
int MMapFile::resize(size_t length) {
  ...
  // If requested size is less than or equal to current size, return directly
  ...
  int fd = -1;  // Cache file descriptor to avoid repeated open/close
  if (!(flags_ & O_ANONYMOUS)) {
    ...
    if (length) {
      ...
      // On non-x86_64 platforms, munmap first then remmap
      munmap();
#endif
      mem_tmp = (!mem_) ?
    ...
    if (fd >= 0) {
      close(fd);  // Close file descriptor uniformly
    }
  } else {  // anonymous memory
    // Anonymous memory doesn't need to open file, use mremap directly

...
TEST_F(TestWALComponents, TestDeleteLogMetricsEntryV2) {
  ...
  std::vector<char> entry_data(sizeof(primary_tag) + sizeof(ts_spans));
  memcpy(entry_data.data(), primary_tag, sizeof(primary_tag));
  memcpy(entry_data.data() + sizeof(primary_tag), ts_spans, sizeof(ts_spans));
  DeleteLogMetricsEntryV2 entry(800, WALLogType::DELETE, 8, WALTableType::DATA_V2,
                                1001, sizeof(primary_tag), 2, entry_data.data(), 1234, 400, 50);
...
TEST_F(TestWALComponents, TestCheckpointEntry_DecodeConstructor) {
  ...
  CheckpointEntry decoded(1200, WALLogType::CHECKPOINT, encoded + sizeof(WALLogType));

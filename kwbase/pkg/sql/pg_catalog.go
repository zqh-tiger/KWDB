				...
				if table.TableType == tree.TemplateTable {
					...
					if len(allChildInfo.InstTableIDs) > 0 {
						for i := range allChildInfo.InstTableIDs {
							if err := addRow(
								defaultOid(table.ID),                          // oid
								tree.NewDName(allChildInfo.InstTableNames[i]), // relname
								namespaceOid,                                  // relnamespace
								oidZero,                                       // reltype (PG creates a composite type in pg_type for each table)
								oidZero,                                       // reloftype (PG creates a composite type in pg_type for each table)
								tree.DNull,                                    // relowner
...
https://www.postgresql.org/docs/9.5/view-pg-indexes.html`,
	// Note: kwdb_oid is an extension of the schema to much more easily map

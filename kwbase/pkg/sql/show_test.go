...
func TestShowCreateTable(t *testing.T) {
	...
	if _, err := sqlDB.Exec(`
		...
		CREATE DATABASE o;
		CREATE TABLE o.foo(x int8 primary key);
	`); err != nil {
		t.Fatal(err)
	}
	...
	i int8,
	j int8,
	k int8 REFERENCES items (c) MATCH FULL,
	...
	FOREIGN KEY (k, l) REFERENCES items (a, b) MATCH FULL ON UPDATE CASCADE
			...
			expect: `CREATE TABLE %s (
	i INT8 NULL DEFAULT 1,
	j INT8 NULL DEFAULT 2,
	k INT8 NULL DEFAULT 3,
	l INT8 NULL DEFAULT 4,
	CONSTRAINT fk_i_ref_items FOREIGN KEY (i, j) REFERENCES items(a, b) ON DELETE SET DEFAULT,

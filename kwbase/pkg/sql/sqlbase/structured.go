...
const (
	...
	// https://www.cockroachlabs.com/blog/sql-in-kwbasedb-mapping-table-data-to-key-value-storage/.
	BaseFormatVersion
	// FamilyFormatVersion corresponds to the encoding described in
	// https://gitee.com/kwbasedb/kwbase/blob/master/docs/RFCS/20151214_sql_column_families.md
	FamilyFormatVersion
	// InterleavedFormatVersion corresponds to the encoding described in
	// https://gitee.com/kwbasedb/kwbase/blob/master/docs/RFCS/20160624_sql_interleaved_tables.md
...
const (
	// MaxPrimaryTagNum is max primary tag number limit
	MaxPrimaryTagNum = 4
	// MaxTagNum is max tag number limit
	MaxTagNum int = 128
)
...
// ErrMissingTags indicates a timeseries table with no tags.
var ErrMissingTags = errors.New("ts table must contain at least 1 tag")

// ErrTooManyTags indicates a timeseries table with too many tags.
var ErrTooManyTags = errors.New("too many tags for ts table")
...
func GetTsTableDescFromID(
	ctx context.Context, txn *kv.Txn, id ID,
) (*TableDescriptor, tree.TableType, error) {
	sTableID, err := GetTmplTableIDByInstID(ctx, txn, uint32(id))
	if err != nil {
		if strings.Contains(err.Error(), "object cannot found") {
			tableDesc, err := GetTableDescFromID(ctx, txn, id)
			if err != nil {
				return nil, 0, err
			}
			return tableDesc, tableDesc.TableType, err
		}
		return nil, 0, err
	}
	tableDesc, err := GetTableDescFromID(ctx, txn, sTableID)
	if err != nil {
		return nil, 0, err
	}
...
// IsTSTable returns true if the TableDescriptor actually describes a
// TS Table resource.
...
func (desc *TableDescriptor) ValidateTable() error {
	...
	// check if tags of time series table created by user are valid
	if desc.IsTSTable() && desc.ID > keys.MaxReservedDescID {
		if err := desc.validateTags(); err != nil {
			return err
		}
	}
...
// validateTags checks if the numerical limit of tags has been exceeded.
func (desc *TableDescriptor) validateTags() error {
	tagNum := 0
...
// CheckTSTableStateValid Checks whether the state of the timeseries table is PUBLIC.
func (desc *TableDescriptor) CheckTSTableStateValid() error {
	//if desc.State == TableDescriptor_ALTER {
	//	return NewAlterTSTableError(desc.Name)
	//}
	if desc.State == TableDescriptor_ADD {
		return NewCreateTSTableError(desc.Name)
	}
	if desc.State == TableDescriptor_DROP {
		return NewDropTSTableError(desc.Name)
	}
	if desc.State == TableDescriptor_ALTER {
		return NewAlterTSTableError(desc.Name)
	}
...
// VisibleColumnsWithTagCol returns all non-hidden columns and target columns.
func (desc *TableDescriptor) VisibleColumnsWithTagCol() []*ColumnDescriptor {
	var cols []*ColumnDescriptor
	for i := range desc.Columns {
		col := &desc.Columns[i]
		if !col.Hidden {
...
// VisibleColumnsWithTagColOrdered returns all non-hidden columns and target columns ordered by data-tag.
func (desc *TableDescriptor) VisibleColumnsWithTagColOrdered() []*ColumnDescriptor {
	var cols, tagCols, dataCols, colsOrdered []*ColumnDescriptor
	for i := range desc.Columns {
		col := &desc.Columns[i]
		if !col.Hidden {
			cols = append(cols, col)
	...
	for _, desc := range cols {
		if desc.IsTagCol() {
			tagCols = append(tagCols, desc)
		} else {
			dataCols = append(dataCols, desc)
		}
	}
	colsOrdered = append(colsOrdered, dataCols...)
	colsOrdered = append(colsOrdered, tagCols...)
	return colsOrdered
}
...
// descriptor was written by older version of kwbase which included the
...
func (desc *ColumnDescriptor) IsPrimaryTagCol() bool {
	return desc.TsCol.ColumnType == ColumnType_TYPE_PTAG
}

// IsOrdinaryTagCol is part of the cat.Column interface.
func (desc *ColumnDescriptor) IsOrdinaryTagCol() bool {
	return desc.TsCol.ColumnType == ColumnType_TYPE_TAG
}
...
const (
	// StorageTuple equal kwdbts::STORAGE_TUPLE
	StorageTuple = 0
	// StoragePageEnd equal kwdbts::STORAGE_PAGE_END
	StoragePageEnd = 1
	// StorageIndependentPage equal kwdbts::STORAGE_INDEPENDENT_PAGE
	StorageIndependentPage = 2
)
...
// Namespace corresponding to system table namespace
type Namespace struct {
	ParentID       uint64
	ParentSchemaID uint64
	Name           string
	ID             uint64
}

// PayloadForDistTSInsert means different primary tag value may need to be sent to different node.
type PayloadForDistTSInsert struct {
	// NodeID node id
...
type SinglePayloadInfo struct {
	// Payload represents multiple payloads to be written to the target node
	Payload []byte
	// RowNum represents multiple rowNum
	RowNum uint32
...
func (p *PayloadForDistTSInsert) Count() uint32 {
	var count uint32
	for _, val := range p.PerNodePayloads {
		count += val.RowNum
	}
	return count
}

// BuildTagHintQuery build only tag query statement through tag names, database name and instance table name
func BuildTagHintQuery(tag []string, dbName string, cTable string) string {
	f := tree.NewFmtCtx(tree.FmtSimple)
	f.WriteString("SELECT /*+STMT_HINT ( ACCESS_HINT(TAG_ONLY,  ")
	...
	f.WriteString(" FROM ")
	f.WriteString(dbName)
	f.WriteString(".")
	f.WriteString(cTable)
	return f.CloseAndGetString()
}

// TagValueToString converts tag value to string
func TagValueToString(d tree.Datum) string {
	var res string
	switch val := d.(type) {

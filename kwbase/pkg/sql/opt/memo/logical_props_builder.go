...
func (b *logicalPropsBuilder) buildTSScanProps(scan *TSScanExpr, rel *props.Relational) {
	...
	// ----------
	if !b.disableStats {
		b.sb.buildTSScan(scan, rel)
	}
}

func (b *logicalPropsBuilder) buildTSInsertSelectProps(
	scan *TSInsertSelectExpr, rel *props.Relational,
) {
	// Cardinality
	// -----------
	// Restrict cardinality based on constraint, FDs, and hard limit.
	...
	// ----------
	if !b.disableStats {
		b.sb.buildTSInsertSelect(scan, rel)
	}
}
...
func (b *logicalPropsBuilder) buildSelectProps(sel *SelectExpr, rel *props.Relational) {
	...
	//Get Cardinality and apply Hint
	switch input := sel.Input.(type) {
	case *ScanExpr:
		b.sb.finalizeFromTableTotalCardinality(input.Flags.TotalCardinality, rel, inputProps)

	case *IndexJoinExpr:
		switch _input := input.Input.(type) {
		case *ScanExpr:
			b.sb.finalizeFromTableTotalCardinality(_input.Flags.TotalCardinality, rel, inputProps)
...
func (b *logicalPropsBuilder) buildJoinProps(join RelExpr, rel *props.Relational) {
	...
	totalCardinality := b.getCardinality(join)
	b.sb.finalizeFromJoinTotalCardinality(totalCardinality, rel)
}
...
func (b *logicalPropsBuilder) buildGroupingExprProps(groupExpr RelExpr, rel *props.Relational) {
	...
	// ----------
	if !b.disableStats {
		b.sb.buildGroupBy(groupExpr, rel, groupPrivate.GroupingCols, groupPrivate.ErrorOnDup)
	}
}
...
func (b *logicalPropsBuilder) buildTSInsertProps(ins *TSInsertExpr, rel *props.Relational) {
	ins.grp.relational().Cardinality = props.OneCardinality
	rel.HasPlaceholder = ins.HasPlaceHolder
}

func (b *logicalPropsBuilder) buildTSDeleteProps(ins *TSDeleteExpr, rel *props.Relational) {
	ins.grp.relational().Cardinality = props.OneCardinality
}

func (b *logicalPropsBuilder) buildTSUpdateProps(ins *TSUpdateExpr, rel *props.Relational) {
	ins.grp.relational().Cardinality = props.OneCardinality
}
...
func (b *logicalPropsBuilder) buildCreateTriggerProps(
	cp *CreateTriggerExpr, rel *props.Relational,
) {
...
// getTotalCardinality return TotalCardinality of a join expression.
func (b *logicalPropsBuilder) getCardinality(join RelExpr) float64 {
	switch j := join.(type) {
	case *MergeJoinExpr:
		return j.MergeJoinPrivate.HintInfo.TotalCardinality
	case *LookupJoinExpr:
		return j.LookupJoinPrivate.HintInfo.TotalCardinality
	case *InnerJoinExpr, *LeftJoinExpr, *FullJoinExpr, *SemiJoinExpr, *AntiJoinExpr:
		return j.Private().(*JoinPrivate).HintInfo.TotalCardinality
	default:
		return 0.0
	}
}

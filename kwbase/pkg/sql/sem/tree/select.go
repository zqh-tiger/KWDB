...
// OrderLimitInfo save deal with order by and limit
...
// CheckHintType return true when have IndexFlags hint.
func (node *SelectClause) CheckHintType() bool {
	for _, table := range node.From.Tables {
		if tb, ok := table.(*AliasedTableExpr); ok {
			if tb.IndexFlags != nil {
				return true
			}
		} else {
			return true
...
func (node *SelectClause) Format(ctx *FmtCtx) {
	ctx.needSource = node.NeedPushDown
	if node.TableSelect {
...
//// SatisfyClause .
//type SatisfyClause struct {
//	Expr Expr
//}
//
//// Format implements the NodeFormatter interface.
//func (s *SatisfyClause) Format(ctx *FmtCtx) {
//	ctx.WriteString("SATISFYING ")
...
func (*InFlightCreateTableExpr) tableExpr() {}

// InFlightCreateTableExpr represents insert into xxx using xxx
type InFlightCreateTableExpr struct {
	TableDef *CreateTable
}

// Format implements the NodeFormatter interface.
func (i *InFlightCreateTableExpr) Format(ctx *FmtCtx) {
	ctx.FormatNode(&i.TableDef.Table)
	i.TableDef.FormatBody(ctx)
}
...
// JoinNodeHint represents a JOIN operation Hint.
type JoinNodeHint struct {
	JoinMethod           keys.JoinMethodHintType
	IndexName            string
	RealOrder            bool
	TotalCardinality     float64
	EstimatedCardinality float64

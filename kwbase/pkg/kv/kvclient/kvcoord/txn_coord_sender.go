...
func (tc *TxnCoordSender) Send(
	...
	if pErr != nil {
		if ba.Txn != nil && pErr.UnexposedTxn != nil {
			tc.mu.txn.TxnMeta.WriteKeyCount += pErr.UnexposedTxn.BatchWriteKeyCount
		}
	}
	...
	if ba.Txn != nil && br != nil && br.Txn != nil {
		tc.mu.txn.TxnMeta.WriteKeyCount += br.Txn.BatchWriteKeyCount
	}
	return br, nil
}
...
// SetUnconsistency is part of the client.TxnSender interface.
func (tc *TxnCoordSender) SetUnconsistency(unConsistency bool) {
	tc.mu.Lock()
	defer tc.mu.Unlock()

	if tc.mu.txn.UnConsistency == unConsistency {
		return
	}
	...
	tc.mu.txn.UnConsistency = unConsistency
}

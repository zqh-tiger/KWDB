...
func (r *Replica) adminSplitWithDescriptor(
	...
	if desc.StartKey.Equal(splitKey) {
		...
		if desc.GetStickyBit().Less(args.ExpirationTime) || args.SplitType == roachpb.TS_SPLIT {
			err := r.store.DB().Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
				return splitTxnStickyUpdateAttempt(ctx, txn, desc, args.ExpirationTime, args)
			})
...
func (r *Replica) AdminMerge(
	...
	runMergeTxn := func(txn *kv.Txn) error {
		...
		if updatedLeftDesc.GetRangeType() != rightDesc.GetRangeType() {
			return errors.Errorf("The range type does not match.")
		}
		log.Infof(ctx, "updatedLeftDesc:%s initiating a merge of %s into this range (%s)", &updatedLeftDesc, &rightDesc, reason)
...
//     technicality (see https://gitee.com/kwbasedb/kwbase/pull/40268).
//
...
//     See https://gitee.com/kwbasedb/kwbase/pull/40268.
//
...
const (
	// NeedTsSnapshotData when need TsSnapshotData
...
func prepareChangeReplicasTrigger(
	...
	rangetyp := roachpb.DEFAULT_RANGE
	if desc.GetRangeType() == roachpb.TS_RANGE {
		rangetyp = roachpb.TS_RANGE
		updatedDesc.RangeType = &rangetyp
	}
...
//	(Fo)WriteSnapshotData
//	(Fo)ApplySnapshot
//	(Fo)DropSnapshot
//	--RESPONSE
//	(LH)DropSnapshot
func (r *Replica) sendTSSnapshot(
	ctx context.Context,
	...
	if err := r.store.cfg.Transport.SendTSSnapshot(
		ctx,
		...
		if errors.Cause(err) == errMalformedSnapshot {
			...
			log.Fatal(ctx, "malformed TS snapshot generated")
		}
		return &snapshotError{err}
	}
	log.Infof(ctx, "snapshot success")
	return nil
}

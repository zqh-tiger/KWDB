...
func (s *Store) processRaftRequestWithReplica(
	...
	drop := maybeDropMsgApp(ctx, (*replicaMsgAppDropper)(r), &req.Message, req.RangeStartKey, req.ReqType)
	if !drop {
...
func (s *Store) processRaftSnapshotRequest(
	...
	if snapHeader.IsPreemptive() {
		return roachpb.NewError(kwdberrors.AssertionFailedf(`expected a raft or learner snapshot`))
	}
...
func (s *Store) processRequestQueue(ctx context.Context, rangeID roachpb.RangeID) bool {
	...
	if hadError {
		...
		// forgiving.
		//
		// See https://gitee.com/kwbasedb/kwbase/issues/30951#issuecomment-428010411.
...
func (s *Store) processReady(ctx context.Context, rangeID roachpb.RangeID) {
	...
	var err error
	if r.isTs() {
		stats, expl, err = r.handleRaftTSReady(ctx, noSnap)
	} else {
...
func (s *Store) updateCapacityGauges() error {
	...
	s.metrics.TsdbUsed.Update(desc.Capacity.TsdbUsed)
	s.metrics.RelationalUsed.Update(desc.Capacity.RelationalUsed)

...
func createTestStoreWithConfig(
	...
	if err := store.Start(context.Background(), stopper, nil); err != nil {
		t.Fatal(err)
	}
	store.WaitForInit()
	return store
...
func TestBootstrapOfNonEmptyStore(t *testing.T) {
	...
	switch err := errors.Cause(store.Start(ctx, stopper, nil)); err.(type) {
...
func TestStoreSendWithClockOffset(t *testing.T) {
	...
	_, err := kv.SendWrappedWith(context.Background(), store.TestSender(), roachpb.Header{Timestamp: reqTS}, &args)
	if err != nil {
		fmt.Println(err)
	}
...
func TestSendSnapshotThrottling(t *testing.T) {
	...
	// Test that a failed Recv() causes a fail throttle
	{
		...
		err := sendSnapshot(ctx, &cfg, st, c, sp, header, &OutgoingSnapshot{}, newBatch, nil)
...
func TestSnapshotRateLimit(t *testing.T) {
	...
	testCases := []struct {
		...
		{SnapshotRequest_REBALANCE, 16 << 20, ""},
	}
...
func TestPreemptiveSnapshotsAreRemoved(t *testing.T) {
	...
	// Start the store.
	require.NoError(t, s.Start(ctx, stopper, nil))

...
// evalAndProposeTS encapsulates TsRequest into a proposal structure and inserts it into proposalBuf
func (r *Replica) evalAndProposeTS(
	...
	// Put it in proposalBuf
	maxLeaseIndex, pErr := r.proposeTS(ctx, proposal)
	if pErr != nil {
		return nil, nil, 0, pErr
	}
	// Abandoning a proposal unbinds its context so that the proposal's client
...
func (r *Replica) stepRaftGroup(req *RaftMessageRequest) error {
	...
	return r.withRaftGroup(false, func(raftGroup *raft.RawNode) (bool, error) {
		...
		if err == raft.ErrProposalDropped {
			...
			// https://gitee.com/kwbasedb/kwbase/issues/21849
			err = nil
		}
...
func (r *Replica) handleRaftTSReady(
	ctx context.Context, inSnap IncomingSnapshot,
) (handleRaftReadyStats, string, error) {
	defer func(start time.Time) {
		elapsed := timeutil.Since(start)
	...
	r.raftMu.Lock()
	defer r.raftMu.Unlock()
	return r.handleRaftTSReadyRaftMuLocked(ctx, inSnap)
}
...
func (r *Replica) handleRaftReadyRaftMuLocked(
	...
	if err := batch.Commit(rd.MustSync && !disableSyncRaftLog.Get(&r.store.cfg.Settings.SV), storage.NormalCommitType); err != nil {
...
// handleRaftTSReadyRaftMuLocked The logic of handling proposals by sequential
// replicas, which requires consensus to be reached in the process
func (r *Replica) handleRaftTSReadyRaftMuLocked(
	ctx context.Context, inSnap IncomingSnapshot,
) (handleRaftReadyStats, string, error) {
	var stats handleRaftReadyStats
...
func (r *Replica) sendRaftMessage(ctx context.Context, msg raftpb.Message) {
	...
	var reqIsTs bool
	if r.Desc().RangeType != nil && *r.Desc().RangeType == roachpb.TS_RANGE {
		reqIsTs = true
	}
